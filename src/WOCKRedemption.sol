...
contract WOCKRedemption {
    ...
    function redeem(uint256[] memory tokenIds) public returns (uint256 streamId) {
        for (uint256 i = 0; i < tokenIds.length; i++) {
            IERC721(nft).safeTransferFrom(msg.sender, address(0xdead), tokenIds[i]);
        }
        uint256 total = tokenIds.length * REDEMPTION;
        uint256 vested = Math.min(total, total / 4 + (3 * (block.timestamp - startTime) * total) / (52 weeks));
        IERC20(asset).safeTransferFrom(treasury, msg.sender, vested);
        if (total - vested > 0) {
            IERC20(asset).safeTransferFrom(treasury, address(this), total - vested);
            ...
            params.durations = LockupLinear.Durations({
                ...
                total: uint40(52 weeks - Math.min(52 weeks, block.timestamp - startTime))
            });
