...
contract Common is Test {
    ...
    function setUpBase() public {
        ...
        pint.approve(address(trisRedemption), ~uint256(1));
        pint.approve(address(wockRedemption), ~uint256(1));
        pint.approve(0xB10daee1FCF62243aE27776D7a92D39dC8740f95, ~uint256(1));
        vm.deal(treasury, 1000 ether);
        pint.approve(address(router), ~uint256(1));
        IERC20(address(weth)).approve(address(router), ~uint256(1));
        ...
        pair = UniswapV2PairComputeLibrary.pairFor(address(factory), address(pint), address(weth));
